...
{
    glm::vec2 middle = entity.getPosition() + entity.getSize()/2.f;
    glm::vec2 bottom = middle + glm::vec2(0.f, entity.getSize().y/2.f);
    entity.getSprite().texCoords = currentFrame->first;
    glm::vec2 newSize = glm::vec2(currentFrame->first.z, currentFrame->first.w);
    entity.setSize(scale * newSize);

    if(origin == Origin::middle)
    {
        entity.setPosition(middle - scale*newSize/2.f);
    }
    else if(origin == Origin::bottom)
    {
        entity.setPosition(bottom - glm::vec2(scale*newSize.x/2.f, scale*newSize.y));
    }
}
